...
check_Test_gene_LR <- function(){
  ...
  colnames(collect_result) = c("Tissue", "Gene", "Variable", "Median_TPM","t_stat", "pvalue")
  collect_result$t_stat = as.numeric(as.character(collect_result$t_stat))
...
plot_gene_sex <- function(df){
  ...
  for(i in seq(1, dim(Gene_SEX)[1])){
    ...
    png(paste0(outdir,'plots/', Test_gene_name, '/', Test_gene_name, '_',tis_name,'_SEX_LR.png'), 
