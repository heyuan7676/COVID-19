...
readin_data_in_tissue <- function(tissue){
  ...
  if(Test_gene %in% rownames(gene_tpm_in_the_tissue)){
   ...
   Test_gene_tpm$geneEXP = as.numeric(gene_tpm_in_the_tissue[which(tpm_matrix_gene_names == Test_gene), samples_used])
  }else{
   return ()
  }
...
check_Test_gene_LR <- function(){
  ...
  for(tissue in sort(unique(samples$SMTSD))){
    ...
    AGE_GROUP_test = c(tissue, Test_gene_name, "AGE", median(as.numeric(exp_for_tiss$geneEXP)), 
                       summary(model)$coefficients[,1]["AGE_GROUP"], 
    ...
    if(sum(table(exp_for_tiss$SEX) < 10) > 0){
      SEX_test = c(tissue, Test_gene_name,"SEX",median(as.numeric(exp_for_tiss$geneEXP)), 0, -1)
    }else if(length(unique(exp_for_tiss$SEX)) == 2){
      SEX_test = c(tissue, Test_gene_name, "SEX", median(as.numeric(exp_for_tiss$geneEXP)),
                   summary(model)$coefficients[,1]["SEX"], 
...
plot_gene_sex <- function(df){
  ...
  for(i in seq(1, dim(Gene_SEX)[1])){
    ...
    png(paste0(outdir,'plots_LR/', Test_gene_name, '/', Test_gene_name, '_',tis_name,'_SEX_LR.png'), 
...
plot_gene_age <- function(df){
  ...
  for(i in seq(1, dim(Gene_AGE)[1])){
    ...
    tis_name = gsub(" ", "_", gsub('\\)', '', gsub(' \\(', '_', gsub(' - ', '_', tissue)))) 
    png(paste0(outdir,'plots_LR/', Test_gene_name, '/', Test_gene_name, '_',tis_name,'_AGE_LR.png'),
        res = 130, height = 500, width = 600)
    print(g_AGE)
    dev.off()
  }
...
dir.create(file.path(outdir, "plots_LR/", Test_gene_name, '/'), showWarnings = FALSE)
reg_result = check_Test_gene_LR()
